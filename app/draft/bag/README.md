...
Unit names come from the bot's JSON text models. The importer corrects the known `tf-dragonfreed` typo to **The Dragon, Freed**, as confirmed by both the `obsidian-warsun2` unit and technology entries in `data/units/newdawn.json` and `data/technologies/newdawn.json`. The bot's Twilight's Fall entry currently spells it "Fred". All 316 bundled unit and unit-upgrade names (83 flagships, 85 mechs, 70 monuments, 47 technology upgrades, and 31 Twilight's Fall upgrades) were compared against the corresponding text entries; the remaining names match after whitespace cleanup. No image recognition is used for this data.
