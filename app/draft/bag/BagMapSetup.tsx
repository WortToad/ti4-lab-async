...
export function BagMapSetup({
  ...
  return (
    <Stack component="section" align="flex-start" gap="sm">
      <Title order={3}>
        {mapRoomId
      ...
      {includesTiles && (
        <List type="ordered" spacing="xs" size="sm">
          <List.Item>
            <strong>During the bag draft, choose which tiles you want.</strong>{" "}
            Each tile uses one of your normal bag picks. You can collect up to{" "}
            {draftBlues} blue and {draftReds} red tiles across all bags, taking
            at most one of each color per pass. Picking a tile adds it to your
            collection. Tile placement happens after the bag draft.
          </List.Item>
          <List.Item>
            <strong>After the bags stop passing, confirm what you keep.</strong>{" "}
            Keep {keepBlues} blue and {keepReds} red tiles alongside your final
            faction components.
          ...
          {!unavailableReason && (
            <List.Item>
              ...
              the builder randomly draws one of your remaining tiles. You choose
              a highlighted space in your section for that tile. Every tile in
              your hand will be placed.
            </List.Item>
          )}
        </List>
      )}
            ...
            <Accordion.Panel>
              <Stack gap="xs">
                <Text size="sm">
                  ...
                  stage 2 spaces, then their two stage 3 spaces, as numbered in
                  the diagram. On your turn, the highlighted spaces show your
                  available choices in the current group. The player with the
                  most empty spaces in that group places next.{" "}
                ...
                <Text size="sm">
                  Your home goes in the home position assigned by your seat. The
                  builder places your chosen home system when supported, or
                  shows a home placeholder for you to use when setting up the
                  game.
                </Text>
                <Text size="sm">
      ...
      {mapRoomId && mapPath ? (
        <>
          <Text size="sm">
            Your map room is ready. Open it below to continue with your current
            player slot and admin access.
          </Text>
        ...
        phase !== "setup" && (
          <Text size="sm">
            Keep using this shared lobby link. Once everyone finishes, an “Open
            map room” button appears with the completed factions and carries
            your slot into the map.
          </Text>
        )
      )}
    </Stack>
